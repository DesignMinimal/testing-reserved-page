...
public class BasePageObject {
	...
	/** Wait for alert to be visible and then switch to it */
	...
	public void dragAndDrop(WebElement from, WebElement to) {
		// Selenium - if it works, we use this, if not - the other
		// Actions action = new Actions(driver);
	...
	protected void clickOnSubMenuElement(By mainMenuLocator, By subMenuLocator) {
		Actions action = new Actions(driver);
		
		//waitForVisibilityOf(mainMenuLocator, Duration.ofSeconds(5));
		action.moveToElement(find(mainMenuLocator)).build().perform();;
		
		//waitForVisibilityOf(subMenuLocator, Duration.ofSeconds(5));
		action.click(find(subMenuLocator)).build().perform();
	}
